...
def define_args()->argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description=(
        "Main script for processing pipeline including " 
    ...
    subparser_train.add_argument("--experiment", type=int, default=1, choices=[1,2,3,4,5,6], help=(
            ...
            "experiment 2: imagine left fist vs imagine right fist\n"
            "experiment 3: both fists vs both feet\n"
            "experiment 4: imagine both fists vs imagine both feet\n"
            "experiment 5: rest vs left fist\n"
            "experiment 6: rest vs imagine both feet\n"
            "Default=1"
        ))
    subparser_train.add_argument("--subject", type=int, default=1, \
        help="subject no. (1 to 109). Default = 1")
    subparser_train.add_argument("--verbose", action='store_true', \
        help="Display additional info on training in terminal")
    subparser_predict = subparser.add_parser("predict", \
        formatter_class=argparse.RawTextHelpFormatter )
    subparser_predict.add_argument("--experiment", type=int, default=1, 
        choices=[1,2,3,4,5,6], help=(
            "experiment 1: left first vs right fist\n"
...
def define_grid()->dict:
    '''return param_grid for GridSearchCV'''
...
def define_pipeline(args:argparse.Namespace) ->Pipeline:
    '''define and return Pipeline Object for GridSearchCV'''
    try:
    ...
    return Pipeline([
        ...
        ('pca',PCA()),
        ("clf", clf)
        ], memory=None, verbose=(True if args.verbose else False))
...
def train(experiment:int, subject:int, args:argparse.Namespace, train_all:bool, indiv_score:list | None)-> None:
    ...
    cv:int = 5
    # combining gridsearch of best param together with cross fold val. 
    # Hence the amount of fitting is no. of combination
    # of parameters x Kfold. As below it's params(2x2) x cv(5) = 20. Hence train model 10 times. 
    grid = GridSearchCV(define_pipeline(args), define_grid(), \
        n_jobs=-1, cv=cv, verbose=(4 if args.verbose else 0))
    ...
    if train_all is False:
        ...
        # parsing cross validation score across the folds.
        # Use index of best params combination - rank 1 in key('rank_test_score')
        # and use that to get score of each fold -> key(ie 'split0', 'split1').
        split:list = []
        for i in range(cv):
            split.append(f"split{i}")
        cv_score:list = [round(float(result[value + '_test_score'] \
            [result['rank_test_score'].tolist().index(1)]),4) for value in split]
        print(cv_score)
...
def predict(args:argparse.Namespace, streaming:bool) -> None:
    ...
    # reading preprocessed file as per args params
    try:
        df:pd.DataFrame = pd.read_csv(f"{PREPROCESSED_PATH}\
S{prefix}{args.subject}E{args.experiment}.csv")
    except Exception as e:
        print(f"An unexpected error occurred: {e}")
        print("exiting..")
        exit()
